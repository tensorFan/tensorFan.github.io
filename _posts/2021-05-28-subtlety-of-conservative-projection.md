...
\section{Conservation of mass in finite elements}
...
Thus it is of \textit{paramount} importance that equation \ref{eq:1} is satisfied, and we must therefore take extreme care in how we approximate $g$ when actually performing the computations. This is a fact which I have found is seldom emphasised.

If\footnote{If we are sampling $g$ from some physical system then I do not think there is any issue, but I lack experience of this so if you know something about it please contact me.} we know $g$ in closed form then we must either evaluate $g$ directly in the quadrature points of the integration, or we must project $g$ to $\partial \Omega$. If we simply interpolate $g$, as is often convenient to do in a FEM code, then we will destroy mass conservation!

\section{Extra: should it matter to which polynomial space I project?}
